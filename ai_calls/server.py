...
from schemas.ai_call_agent_schemas import Scenario, Agent, CallStatus
...
async def outbound_call(request: OutboundCallRequest):
    ...
    call = twilio_client.calls.create(
        ...
        to=request.to,
        from_=request.from_,
        # twiml is twilio markup language, a set of instructions you give twilio
        # to tell it what to do during phone call/text message interaction
        # written in XML. In this case, we tell twilio to set up a 2-way stream to 
        # a websocket URL, in this case, of the format "wss://xxx.ngrok.io/ws", 
        # to connect the calls audio to the websocket here, and
        # then pause the call for 10 seconds after starting the stream (user answers)
